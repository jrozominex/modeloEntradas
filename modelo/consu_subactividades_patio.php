...
if($_POST['band'] == 1){
    ...
    while ($actividad = sqlsrv_fetch_array($resul)){
    	...
    	<center><button type="button" id="'.$idActividad.'_edit" class="btn btn-default" value="'.$idActividad.'" onclick="actividad_edit('.chr(34).$idActividad.chr(34).')">'.$descripcion.'</button>
            </center>
        </div>';
    }
	$datos =$texto."||". 
	...
	{	$sub_actividad = Array();
		...
		{	$a = implode(",", $sub_actividad);
			$sql = "SELECT * FROM subactividades_cargadores WHERE idActividad='$idActividad' AND Descripcion IN ('APILAR', 'ALIMENTAR', 'CARGAR DESPACHO', 'STANDBY', 'MVTO. X CALIDAD', 'OFICIOS VARIOS', 'TRABAJOS MCOS.') order by Descripcion";
			$params = array();
		    $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET);
		    $res=sqlsrv_query($conn,utf8_decode($sql),$params,$options);
		    $rows2=sqlsrv_num_rows($res);
			...
			$sql = "SELECT * FROM subactividades_cargadores WHERE idActividad='$idActividad' AND Descripcion IN ('APILAR', 'ALIMENTAR') order by Descripcion";
			$params = array();
		    $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
		    $res=sqlsrv_query($conn,utf8_decode($sql),$params,$options);
		    $rows2=sqlsrv_num_rows($res);
	...
	*/
	...
	//
	$ProductoObjetivo = $_POST['ProductoObjetivo'];
	//
	...
	{ 	while($rowss = sqlsrv_fetch_array($result1)){
		...
		//if($total_apilar < $TM_alimentadas){
			...
			if ($total_resta > $TempMaquinaEst){
				...
				$sql = "INSERT INTO tiempos_cargadores_actividad(idDistribucion, idMaquinaria, idSubActividad, idClasificacion, idClasificacionObjetivo, cantidad, TMxPalada, tiempohorometro, idRegistro, TM_total, ajuste_TM, idPila, idEquipo, idZona, horas_clasif, tipo_tarifa)
						VALUES (NEWID(),'$maquinaria','$sub_actividad','$producto','$ProductoObjetivo','$n_paladas','$TM_palada','$TempMaquinaEst','$idRegistro','$TotalTM','$ajuste','$pila','$equipo','$Zona','$tempo_clasif','0')";
	...
	{	//ECHO "PASÓ";
		...
		if ($total_resta > $calculado){
			...
			if($TempMaquinaEst != 0){
				/*$sql = "UPDATE tiempos_cargadores_actividad SET cantidad='$n_paladas', TMxPalada='$TM_palada', tiempohorometro='$TempMaquinaEst',TM_total='$TotalTM'
						WHERE idDistribucion='$idDistribucion'";
				$sql_insert = "UPDATE horometro SET total_horas='$TempMaquinaEst', fecha_cierre_horometro='$fecha'
				...
				tiempohorometro='$TempMaquinaEst',TM_total='$TotalTM', idPila='$pila', idEquipo='$equipo', idZona='$zona', horas_clasif='$tempo_clasif'
						WHERE idDistribucion='$idDistribucion'";
				$sql_insert = "UPDATE horometro SET total_horas='$TempMaquinaEst', fecha_cierre_horometro='$fecha', idClasificacion='$producto' WHERE id_horometro='$idHorometro'";
			}else{
			...
			if ($resul1){
				if($TempMaquinaEst != 0){
					$sql_insert = "UPDATE horometro SET total_horas='$TempMaquinaEst', fecha_cierre_horometro='$fecha' WHERE id_horometro='$idHorometro'";
				}else{
					$sql_insert = "DELETE FROM horometro WHERE id_horometro='$idHorometro'";
				}
				$result = sqlsrv_query($conn,$sql_insert);
	...
	/********************************************************************************************************************************/
		if ($calculado < $total_resta){
			...
			if($TempMaquinaEstVehiculo != 0){
				$sql = "UPDATE tiempos_cargadores_actividad SET cantidad='$n_vehiculos', tiempohora='$TempReEstVehiculo', tiempopromedio='$TempXvehiculo', tiempohorometro='$TempMaquinaEstVehiculo'
						WHERE idDistribucion='$idDistribucion'";
			}else{
				$sql = "DELETE FROM tiempos_cargadores_actividad WHERE idDistribucion='$idDistribucion'";
			}
			...
			if ($resul1){
				if($TempMaquinaEstVehiculo != 0){
					$sql_insert = "UPDATE horometro SET total_horas='$TempMaquinaEstVehiculo', fecha_cierre_horometro='$fecha'
									WHERE id_horometro='$idHorometro'";
				}else{
					$sql_insert = "DELETE FROM horometro WHERE id_horometro='$idHorometro'";
				}
	...
	/********************************************************************************************************************************/
		if ($calculado < $total_resta){
			...
			if($time_est != 0){
				/*$sql = "UPDATE tiempos_cargadores_actividad SET tiempohorometro='$time_est', TM_total='$TotalTM'
						WHERE idDistribucion='$idDistribucion'";*/
			...
			if ($resul1){
				if($time_est != 0){
					/*$sql_insert = "UPDATE horometro SET total_horas='$time_est', fecha_cierre_horometro='$fecha'
									WHERE id_horometro='$idHorometro'";*/
	...
	$idRegistro = $_POST['idRegistro'];
	$Year = date('Y')-1;
	$sql1 = "SELECT * FROM tiempos_cargadores_actividad WHERE idSubActividad IN 
		(SELECT idSubactividad FROM subactividades_cargadores WHERE Descripcion='ALIMENTAR') AND idRegistro='$idRegistro'";
	$params = array();
    $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
    $res1=sqlsrv_query($conn,utf8_decode($sql1),$params,$options);
    $rows=sqlsrv_num_rows($res1);
    $fill = 0;
    $fill2 = 0;
    $position = 0;
    $mensaje = '';
    if($rows > 0){
		while($act = sqlsrv_fetch_array($res1)){
			$Array_subAct[$fill] = "'".$act['idClasificacion']."'";
			$Array_subAct2[$fill2] = "'".$act['idClasificacionObjetivo']."'";
			$fill++;
			$fill2++;
		}
		$Array_subAct = implode("','", $Array_subAct);
		$Array_subAct2 = implode("','", $Array_subAct2);
		$sql2 = "SELECT * FROM ClasificacionProductos WHERE idClasificacionA in($Array_subAct) AND idClasificacionO in ($Array_subAct2)";
		$res = sqlsrv_query($conn,$sql2);
		while($a = sqlsrv_fetch_array($res)){
			$Array_id[$position] = "'".$a['id']."'";
		}
		$Array_id = implode("','", $Array_id);
		$mensaje .= '<option value="00000000-0000-0000-0000-000000000000" selected>Seleccione</option>';
		/*$sql = "SELECT Clasificacion.idClasificacion, Clasificacion.Descripcion
			FROM ClasificacionProductosDetalle INNER JOIN
			Clasificacion ON ClasificacionProductosDetalle.idClasificacion = Clasificacion.idClasificacion
			WHERE id in ($Array_id) ORDER BY Clasificacion.Descripcion";*/
		$sql = "SELECT DISTINCT(Clasificacion) as Descripcion,Clasificacion.idClasificacion FROM tz_MovimientoTransporte  
                INNER JOIN Clasificacion ON tz_MovimientoTransporte.Clasificacion=Clasificacion.Descripcion
                WHERE year(FechaRegistro)>='$Year' ORDER BY Clasificacion";
		$res = sqlsrv_query($conn,$sql);
		while($qa = sqlsrv_fetch_array($res)){
			$nombre = $qa['idClasificacion'];
			//Output
			$mensaje.= '<option value="'.$nombre.'">'.utf8_encode($qa['Descripcion']).'</option>';
		}
		echo $mensaje;
	}
